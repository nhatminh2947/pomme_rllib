...
class SimplePolicy(Policy):
    ...
    def obs_to_dict(self, obs):
        ...
        id += board_size ** 2
        obs_dict["bomb_blast_strength"] = obs[id: id + board_size ** 2].reshape((board_size, board_size))

        id += board_size ** 2
        obs_dict["bomb_life"] = obs[id: id + board_size ** 2].reshape((board_size, board_size))

        id += board_size ** 2
        obs_dict["position"] = (int(obs[id]), int(obs[id + 1]))

        id += 2
        obs_dict["ammo"] = obs[id]

        id += 1
        obs_dict["blast_strength"] = obs[id]

        id += 1
        obs_dict["can_kick"] = obs[id]

        id += 1
        obs_dict["teammate"] = obs[id]

        id += 1
        obs_dict["enemies"] = np.asarray([obs[id], obs[id + 1], obs[id + 2]])
    ...
    def compute_actions(self, obs_batch, state_batches=None, prev_action_batch=None, prev_reward_batch=None,
        ...
        return [self.act(obs) for obs in obs_batch], [], {}
