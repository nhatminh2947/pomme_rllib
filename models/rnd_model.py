...
class RNDActorCriticModel(nn.Module, TorchModelV2):
    ...
    def compute_intrinsic_reward(self, next_obs):
        next_obs = torch.from_numpy(next_obs).cuda()

        target_next_feature = self.target(next_obs)
        predict_next_feature = self.predictor(next_obs)
        intrinsic_reward = (target_next_feature - predict_next_feature).pow(2).sum(1) / 2
        # ir = F.mse_loss(target_next_feature, predict_next_feature, reduction='none').sum(1)

        return np.asarray(intrinsic_reward.data.tolist())
