...
from agents.neoteric.enums import Board, Action, Direction, SensorType
from agents.neoteric.sensors.sensor import Sensor
from agents.neoteric.utils import move_in_direction
...
class Network:
            ...
            if enemy_pos[0] == position[0]:
                for y in range(1 + min(enemy_pos[1], position[1]), max(enemy_pos[1], position[1])):
                    ...
                    if ticks[enemy_pos[0]][y]:
                        ...
                        aligned_bomb_pos = None
                        break
            if aligned_bomb_pos and self._distance(aligned_bomb_pos, position) > self._distance(aligned_bomb_pos,
                                                                                                enemy_pos):
                aligned_bomb_pos = None
    ...
    def _forward(self, position):
        ...
        while not fque.empty():
            ...
            for direction, action in zip([Direction.UP, Direction.LEFT, Direction.DOWN, Direction.RIGHT],
                ...
                if self._exists(next_position) and (
                        self._get(next_position).isOccupiable or self._get(next_position).isBomb):
    ...
    def _getSafeandTurns(self):
        reachable_safe_nodes = list(filter(lambda node: node.isSafe and node.isOccupiable and node.reach < UNREACHABLE,
                                           list(self.flatten(self.grid)) + self.spare))
...
class NetworkSensor(Sensor):
    ...
    def sense(self, obs):
        ...
        return self.network.reflect(self.position, board.type(torch.LongTensor), radius, ticks, flares, enemies,
                                    teammate, alive, canKick)
