...
from agents.neoteric.enums import Board, Action, SensorType
from agents.neoteric.sensors.network_sensor import NetworkMethods as tm
from agents.neoteric.sensors.network_sensor import NetworkSensor
from agents.neoteric.sensors.sensor import Option
from agents.neoteric.utils import Result
...
class NeotericAgent(agents.BaseAgent):
    ...
    def __init__(self, original=True, *args, **kwargs):
        ...
        self.safety = Option(SensorType.NETWORK, [tm.filter_safe, tm.filter_reachable, tm.filter_occupiable],
                             tm.choose_random_top_k(8))
        self.evade = Option(SensorType.NETWORK,
                            [tm.filter_safe, tm.filter_reachable, tm.filter_non_enemy, tm.filter_occupiable],
                            tm.choose_nearest)
        self.demolish = Option(SensorType.NETWORK,
                               [tm.filter_safe, tm.filter_reachable, tm.filter_breach, tm.filter_occupiable],
                               tm.choose_nearest)
        self.collect = Option(SensorType.NETWORK, [tm.filter_safe, tm.filter_reachable, tm.filter_powerups],
                              tm.choose_nearest)
        self.kick = Option(SensorType.NETWORK, [tm.filter_bomb, tm.filter_root, tm.filter_reachable],
                           tm.choose_nearest)
        self.explore = Option(SensorType.NETWORK, [tm.filter_fog, tm.filter_safe, tm.filter_reachable],
                              tm.choose_nearest)
        self.block = Option(SensorType.NETWORK,
                            [tm.filter_enemy, tm.filter_reachable, tm.filter_safe, tm.filter_occupiable],
                            tm.choose_nearest)
        self.majority_safe = Option(SensorType.NETWORK, [tm.filter_safe, tm.filter_reachable, tm.filter_occupiable,
                                                         tm.filter_reach_by_k(10)],
    ...
    def act(self, obs, action_space):
        ...
        deflect = Option(SensorType.NETWORK, [tm.filter_safe, tm.filter_reachable, tm.filter_occupiable,
                                              tm.filter_not_with_action(self.prev_action)], tm.choose_nearest)
        ...
        if self.original and enemy_loc:
            ...
            potential_hit = (future_state.totalSafe <= 0) or (
                    state.canKick and future_state.totalSafe <= 2 and current_state.totalSafe > 2)
        ...
        if self.original and state.isFollower and obs['message'][0] and obs['message'][
            0] < 6 and self.networkSensor.network._get(
            decoded_location).reachable() and self.networkSensor.network._get(decoded_location).reach > 3:
        ...
        # elif safety_action and state.currentlyNotSafe: # and can-kick and enemy on the line-of-fire and teammate isn't
            ...
            self.curr_action = kick_action
        elif block_action and ((block_action is Action.STOP) or potential_hit) and state.totalTurns > 0 and (
                not state.nextToTeammate or teammate_safe):
