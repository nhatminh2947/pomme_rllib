...
import podman
import requests
from pommerman import characters, utility
from pommerman.agents import BaseAgent


class PodmanAgent(BaseAgent):
    ...
    def __init__(self,
                 ...
                 podman_client=None,
                 env_vars=None):
        super(PodmanAgent, self).__init__(character)

        self._docker_image = docker_image
        self._podman_client = podman_client
        if not self._podman_client:
            self._podman_client = podman.from_env()
            self._podman_client.login(
    ...
    def _run_container(self):
        ...
        self._container = self._podman_client.containers.run(
            self._docker_image,
            detach=True,
            auto_remove=True,
    ...
    def init_agent(self, id, game_type):
        super(PodmanAgent, self).init_agent(id, game_type)
